...
const logger = (() => {
  ...
  const logInfo = (_message: string, _properties: { [key: string]: any }) => {
    if (logLevel <= 2) {
  ...
  // use to log warnings
  const logWarning = (_message: string) => {
    if (logLevel <= 3) {
      //   appInsights.trackTrace({
  ...
  const logAppEvent = (_name: string, _properties?: { [key: string]: any }) => {
    if (logLevel <= 5) {
  ...
  const logAppMetric = (_name: string) => {
